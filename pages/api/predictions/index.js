...
const maskSize = 256;
const version = "c11bac58203367db93a3c552bd49a25a5418458ddffb7e90dae55780765e26d6";
...
export default async function handler(req, res) {
  ...
  const { image, mask, width, height } = await generateImageAndMask(neighborTop, neighborLeft);
...
async function generateImageAndMask(topUrl, leftUrl) {
  ...
  if (topUrl && leftUrl) {
    ...
    ctx.drawImage(topImage, 0, topImage.height - maskSize, 768, maskSize, maskSize, 0, 768, maskSize);
    maskCtx.fillStyle = 'black';
    maskCtx.fillRect(maskSize, 0, 768, maskSize);

    const leftImage = await fetchImage(leftUrl);
    ctx.drawImage(leftImage, leftImage.width - maskSize, 0, maskSize, 768, 0, maskSize, maskSize, 768);
    maskCtx.fillStyle = 'black';
    maskCtx.fillRect(0, maskSize, maskSize, 768);
  } else if (topUrl) {
    const topImage = await fetchImage(topUrl);
    ctx.drawImage(topImage, 0, topImage.height - maskSize, 768, maskSize, 0, 0, 768, maskSize);
    maskCtx.fillStyle = 'black';
    maskCtx.fillRect(0, 0, 768, maskSize);
  } else if (leftUrl) {
    const leftImage = await fetchImage(leftUrl);
    ctx.drawImage(leftImage, leftImage.width - maskSize, 0, maskSize, 768, 0, 0, maskSize, 768);
    maskCtx.fillStyle = 'black';
  ...
  */
  ...
  return { image: imageDataUrl, mask: maskDataUrl, width: canvasWidth, height: canvasHeight };
}
